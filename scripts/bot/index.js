...
async function run() {
  if (comment.startsWith("/assign me") || comment.startsWith("/claim")) {
    try {
      ...
      console.error(e);
      await postComment(
        `❌ Failed to assign @${author}. Make sure they are a collaborator.`
      );
    }
  }
  ...
  if (comment.startsWith("/label") || comment.startsWith("/add label")) {
    const labels = parseLabels(
      comment.startsWith("/label") ? "/label" : "/add label"
    );
    if (labels.length > 0) {
      try {
        ...
        console.error(e);
        await postComment(
          `❌ Failed to add labels. Make sure they exist in the repository.`
        );
      }
    } else {
      await postComment(
        `⚠️ Please specify labels. Example: \`/label bug, frontend\``
      );
    }
  }
  ...
  if (isPR) {
    ...
    if (comment.startsWith("/rebase")) {
      try {
        ...
        await postComment(
          "🔄 Branch update with latest base has been triggered."
        );
      } catch (error) {
        console.error(error);
        await postComment(
          "❌ Failed to rebase. Does the PR have merge conflicts?"
        );
      }
    }
    ...
    if (comment.startsWith("r?") || comment.startsWith("/reviewer")) {
      ...
      if (!matches || matches.length === 0) {
        await postComment(
          "⚠️ Please mention at least one reviewer. Example: `/review @username`"
        );
        return;
      }
      ...
      try {
        ...
        await postComment(
          `✅ Review successfully requested from: ${matches.join(", ")}`
        );
      } catch (error) {
        console.error(error);
        await postComment(
          `❌ Failed to request review. Make sure the users ${matches.join(", ")} have access to this repository.`
        );
      }
    }
    ...
    if (comment.startsWith("/promote")) {
      ...
      await postComment(
        "🚀 Promotion requested: will merge dev → master when CI passes."
      );
    }
