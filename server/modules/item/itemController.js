...
const getItemsForHAB = async (req, res) => {
  ...
  try {
    const itemsWithHAB = await Item.find({
      $and: [{ hostel: hostel }, { currentAuthority: "hab" }],
    });
...
const resolveItem = async (req, res) => {
  ...
  try {
    ...
    console.log(err);
    res.status(500).json({ message: "Error occurred in resolving item" });
  }
};
...
const inProgressItem = async (req, res) => {
  ...
  try {
    ...
    console.log(err);
    res.status(500).json({ message: "Error occurred in in progress item" });
  }
};
