...
function MealStatistic({ meal, stats, date }) {
  ...
  const attendance =
    ((stats[currmeal] / stats.total) * 100).toFixed(1).toString() + "%";
        ...
        <div className="mt-6 grid grid-cols-2">
              ...
              datasets: [
                {
                  ...
                  backgroundColor: [colormap[rank][1], colormap[rank][0]],
                },
              ],
            }}
            options={{
          ...
          <div>
            ...
            <div className="flex gap-8">
              <div
                className={`flex flex-1 gap-4 ${colormap[rank][2]} p-4 items-center rounded-2xl mb-4`}
              >
                <div className={`p-2 ${colormap[rank][3]} rounded-lg`}>
                  <LineChartOutlined
                    className={`${colormap[rank][4]} text-2xl`}
                  />
                </div>
                <div>
            ...
            <div className="text-sm mb-5 text-gray-500">
              {timings[currmeal][index]}
            </div>

            <div className="font-medium mb-1">Breakdown</div>
            <div className="text-sm text-gray-500">
              Students who ate: {stats[currmeal]}
            </div>
            <div className="text-sm mb-4 text-gray-500">
              Students who skipped: {stats.total - stats[currmeal]}
            </div>
          </div>
        </div>
      </section>
